		    ...
		    if($can_edit_emails){ ?>
			    ...
			    if(count($to) > 1){
                    ...
                    <select name="custom_to">
					    ...
					    <?php foreach($to as $t){ ?>
                            <option value="<?php echo htmlspecialchars($t['email']);?>||<?php echo htmlspecialchars($t['name']);?>||<?php echo (int)$t['user_id'];?>"<?php if(isset($options['to_select'])&&$to_select==$t['email'])echo ' selected';?>><?php echo htmlspecialchars($t['email']) . ' - ' . htmlspecialchars($t['name']);?></option>
					    <?php } ?>
                    </select>
			    <?php }else{
				    $t = array_shift($to);
