...
;
    ...
    public static function inner_content_start(){
	    ...
	    if(count($unsent_invoices)){
            ...
            Notice: there are <?php echo count($unsent_invoices);?> unsent subscription invoices due to a recent bug. Please check all your recent invoices and manually send them to the customer if required: <?php echo implode(', ',$unsent_invoices); ?>.
                &nbsp;
                &nbsp;
                &nbsp;
                &nbsp;
                    ...
                    if($next_due_time <= strtotime(date('Y-m-d')) && !$next_due_time_invoice_created){
	                    ...
	                    if($invoice_id){
		                    ...
		                    if($new_invoice_data['customer_id']){
			                    ...
			                    $hash = md5(serialize(array($new_invoice_data['tax_type'],$tax_hash,$new_invoice_data['currency_id'])));
			                    ...
			                    if($debug) echo " - added invoice to customer id ".$new_invoice_data['customer_id']." list for later merging <br>\n";
			                    $subscription_invoices_by_customer[$new_invoice_data['customer_id']][$hash][] = array(
				                    'invoice_id' => $invoice_id,
		        ...
		        foreach($invoice_hashes as $invoice_hash => $invoice_details){
		        	...
		        	if(count($invoice_details) >= 1) {
				        ...
				        if($parent_invoice_id && $automatic_email){
					        ...
					        if($debug) echo " - emailing invoice ".module_invoice::link_open($parent_invoice_id,true)."... <br>\n";
