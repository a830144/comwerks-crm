	...
	$columns['timer_action'] = array(
		...
		'callback' => function ( $timer ) {
			...
			if( $ucmtimer->timer_status == _TIMER_STATUS_RUNNING && module_timer::can_i('edit','Timers')) {
			    ...
			    if($latest) {
				    ?>
                    <a href="<?php echo $ucmtimer->link_open( false ); ?>&_process=complete_timer&timer_segment_id=<?php echo (int) $latest['timer_segment_id']; ?>&form_auth_key=<?php echo module_form::get_secure_key(); ?>"
