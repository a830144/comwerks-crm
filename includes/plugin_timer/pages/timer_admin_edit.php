...
<form action="<?php echo module_timer::link_open($timer_id);?>" method="post">
    ...
    <input type="hidden" name="return" value="<?php echo !empty($_REQUEST['return']) ? htmlspecialchars($_REQUEST['return']) : ''; ?>" />
                ...
                'fields' => array(
                    ...
                    array(
                        ...
                        'lookup' => array(
                            ...
                            'return_link' => true,
                            'owner_table' => $timer['owner_table'], // so our form.php file can render display value correctly. see timer.php autocomplete_display()
                            'display' => '',
                        ),
		    ...
		    'callback' => function($timer_segment) use ($timer){
		        if($timer_segment['timer_status'] == _TIMER_STATUS_PAUSED) {
		            if( module_timer::can_i('delete','Timers')) {
			            ?>
                        <a href="<?php echo $timer->link_open( false ); ?>&delete_segment=<?php echo (int) $timer_segment['timer_segment_id']; ?>"
                           data-ajax-modal='{"type":"normal","title":"<?php _e( 'Delete' ); ?>"}'><i
                                    class="fa fa-times-circle-o"></i></a>
                        &nbsp; &nbsp;  <a href="<?php echo $timer->link_open( false ); ?>&timer_segment_id=<?php echo (int) $timer_segment['timer_segment_id']; ?>"
			        ...
			        if( module_timer::can_i('edit','Timers')) {
				        ?>
                        <a href="<?php echo $timer->link_open( false ); ?>&_process=complete_timer&timer_segment_id=<?php echo (int) $timer_segment['timer_segment_id']; ?>&form_auth_key=<?php echo module_form::get_secure_key(); ?>"
