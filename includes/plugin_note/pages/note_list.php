...
if (isset($options['title']) && $options['title']){
	...
	?>
	<a href="<?php echo module_note::link_open('new',false,$link_options);?>" class="uibutton note_options_link note_add no_permissions"><?php _e('Add New Note');?></a>
	<?php
...
$columns['description'] = array(
    ...
    'callback' => function($note_item){
        ...
        if($note_item['can_edit']){
            ...
            ?>
        <a href="<?php echo module_note::link_open($note_item['note_id'],false,$note_item['options']);?>" rel="<?php echo $note_item['note_id'];?>" data-ajax-modal='{"type":"normal","title":"<?php _e('Note');?>","load_callback":"note_popup_opened","buttons":{"<?php _e('Save note');?>":"save_note_callback","<?php _e('Cancel');?>":"close"}}'> <?php echo $note_text; ?> </a>
        <?php }else{
    ...
    $columns['del'] = array(
        ...
        'callback' => function($note_item){
            if($note_item['can_delete']){
                ?> <a href="<?php echo module_note::link_open($note_item['note_id'],false,array_merge($note_item['options'],array('do_delete'=>'yes','note_id'=>$note_item['note_id'])));?>" data-options="<?php echo htmlspecialchars(base64_encode(serialize(array_merge($note_item['options'],array('do_delete'=>'yes','note_id'=>$note_item['note_id'])))));?>" rel="<?php echo $note_item['note_id'];?>" onclick="if(confirm('<?php _e('Really Delete Note?');?>'))return true; else return false;" class="note_delete note_options_link delete"><i class="fa fa-trash"></i></a> <?php
            }
        }
    );
}
