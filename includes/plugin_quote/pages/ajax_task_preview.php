...
<?php }
            ...
            if(isset($task_data['long_description']) && $task_data['long_description'] != ''){ ?>
                ...
                <div class="task_long_description" <?php if(module_config::c('quote_tasks_show_long_desc',0)){ ?> style="display:block;" <?php } ?>><?php
        ...
        <td align="center" valign="top">
            <?php if($task_editable){ ?>
                <?php if(module_config::c('quote_task_edit_icon',0)){ // old icon:  ?>
                    <a href="#" class="edit-task" title="<?php _e( 'Edit' );?>" onclick="edittask(<?php echo $quote_task_id;?>,<?php echo ($task_data['hours']>0?($task_data['hours']):1);?>); return false;"><i class="fa fa-pencil"></i></a>
                <?php }else{ ?>
